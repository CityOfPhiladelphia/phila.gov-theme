...
<form role="search" method="get" class="search-form pure-form" action="<?php echo home_url( '/' ); ?>">
	<label>
		...
		<input type="search" class="search-field" placeholder="<?php echo esc_attr_x( 'I\'m looking for...', 'placeholder' ) ?>" value="<?php echo get_search_query() ?>" name="s" title="<?php echo esc_attr_x( 'Search for:', 'label' ) ?>" id="search-form"/>
	</label>
